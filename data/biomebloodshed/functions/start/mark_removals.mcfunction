...
execute as @s[tag=!bb.ToRemove] run function biomebloodshed:start/generate_island
execute as @s[tag=bb.ToRemove] run summon marker ~ ~ ~ {Tags:["bb.ToRemove","bb.IslandSpawner","bb.active"]}
kill @s[tag=bb.ToRemove]
execute as @e[type=marker,x=0,tag=bb.active,limit=1] at @s run function biomebloodshed:start/randomize_position
